...
rule run_maker:
    ...
    shell:
        ...
        until [[ ! -e {wildcards.lane_number}.maker.output/{wildcards.lane_number}_datastore ]]
        do
            sleep 60
            /bin/rm -rf {wildcards.lane_number}.maker.output 
        done
        ...
        [[ $(tail -n1 {wildcards.lane_number}.maker.output/{wildcards.lane_number}_master_datastore_index.log) =~ "FINISHED" ]] && \
        [[ $(tail -n10 AnnoSmk/{PREFIX}/R{wildcards.round}/{wildcards.lane_number}.error.maker) =~ "Maker is now finished" ]]
        do
            if [[ -f "{wildcards.lane_number}.maker.output/{wildcards.lane_number}_master_datastore_index.log" ]] && \
            [[ $(tail -n1 {wildcards.lane_number}.maker.output/{wildcards.lane_number}_master_datastore_index.log) =~ "STARTED" ]];then                
                echo "$(date) but unfinish" >> AnnoSmk/{PREFIX}/R{wildcards.round}/{wildcards.lane_number}.log 
                sleep 60
            elif [[ -f "{wildcards.lane_number}.maker.output/{wildcards.lane_number}_master_datastore_index.log" ]] && \
            [[ $(tail -n1 {wildcards.lane_number}.maker.output/{wildcards.lane_number}_master_datastore_index.log) =~ "FAILED" ]];then 
