...
rule RepeatModeler:
    ...
    shell:
        '''
        RepeatModeler -pa 3 -engine ncbi -pa 100 -database {params.db} {input.fa}
...
rule bedtools1:
    ...
    log:
        "result/{PREFIX}/evidence/logs/bedtools.log"
    shell:
        '''
...
# rule cat_est_rnaseq:
#     input:
#         expand("{PREFIX}/{sample}.clean.fq.gz_trinity/Trinity-GG.fasta",PREFIX=PREFIX,sample=samples_trans)
#     output:
#         "result/{PREFIX}/evidence/rnaseq.fasta"
#     shell:
#         "python -m iga.annotation.maker cat_est {input} > {output}"
...
rule cat_gff:
    input:
        expand("result/{PREFIX}/evidence/{sample}.fasta.rawgff.gff",PREFIX=PREFIX,sample="rnaseq")
    output:
